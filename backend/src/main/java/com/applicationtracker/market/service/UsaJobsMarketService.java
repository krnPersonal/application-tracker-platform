...
public class UsaJobsMarketService {
    private static final Logger log = LoggerFactory.getLogger(UsaJobsMarketService.class);
    ...
    public MarketFeedResponse getUsEngineeringMarketFeed() {
        if (userAgent.isBlank() || apiKey.isBlank()) {
            log.warn("USAJOBS credentials missing. userAgentPresent={}, apiKeyPresent={}",
                    !userAgent.isBlank(), !apiKey.isBlank());
        ...
        try {
            ...
            if (response.statusCode() < 200 || response.statusCode() >= 300) {
                log.warn("USAJOBS returned non-success status {}. Body preview: {}",
            ...
            if (jobs.isEmpty()) {
                log.warn("USAJOBS returned 200 but no matching jobs after parsing/filtering.");
                return fallback("USAJOBS returned no matching US engineering roles. Showing a curated US engineering snapshot.");
            }

            log.info("USAJOBS returned {} roles for live market feed.", jobs.size());
            ...
            log.error("USAJOBS live feed request failed: {}", exception.getMessage(), exception);
            return fallback(DEFAULT_ERROR + " " + exception.getMessage());
        }
    }
